...
public final class ChineseCalendar extends GregorianCalendar {
    ...
    public static final int CHINESE_HOUR = 809;
    public static final int CHINESE_MIN = 810;
    ...
    public ChineseCalendar(int y, int m, int d, int h, int min) {
        super(y, m, d, h, min);
    }
    ...
    public ChineseCalendar(boolean isChinese, int y, int m, int d, int hour, int min) {
        if (isChinese) {
            set(CHINESE_YEAR, y);
            set(CHINESE_MONTH, m);
            set(CHINESE_DATE, d);
            set(CHINESE_HOUR, hour);
            set(CHINESE_MIN, min);
        } else {
            set(y, m, d, hour, min);
        }
    ...
    public void set(int field, int value) {
        ...
        if (isChineseField(field)) {
            switch (field) {
                ...
                case CHINESE_HOUR:
                    chineseHour = value;
                    this.set(HOUR_OF_DAY, chineseHour);
                    break;
                case CHINESE_MIN:
                    chineseMin = value;
                    this.set(MINUTE, chineseMin);
                    break;
                default:
    ...
    public String getChinese(int field) {
        ...
        switch (field) {
            ...
            case CHINESE_HOUR:
                return branchTimes[(chineseHour + 1) / 2] + "时";
            case CHINESE_MIN:
                return String.valueOf(chineseMin);
            default:
                throw new IllegalArgumentException("field not supported, field : " + field);
        }
    }
    ...
    public String getSimpleChineseDateString() {
        return new StringBuffer()
                ...
                .append(getChinese(CHINESE_HOUR))
                .append(getChinese(CHINESE_MIN)).toString();
    }
    ...
    private void computeChineseFields() {
        ...
        int gregorianHour = internalGet(Calendar.HOUR_OF_DAY);
        int gregorianMin = internalGet(Calendar.MINUTE);
        ...
        //hour
        chineseHour = gregorianHour;

        //min
        chineseMin = gregorianMin;
    ...
    private static final String[] branchNames = { "", "子", "丑", "寅", "卯", "辰",
            "巳", "午", "未", "申", "酉", "戌", "亥" };
    private static final String[] branchTimes = {"子", "丑", "寅", "卯", "辰",
            "巳", "午", "未", "申", "酉", "戌", "亥", "子"};
