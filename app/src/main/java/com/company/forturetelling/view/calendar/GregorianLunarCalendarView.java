...
public class GregorianLunarCalendarView extends LinearLayout implements NumberPickerView.OnValueChangeListener {
    ...
    private static final int MIN_STOP = 59;
    private static final int MIN_SPAN = MIN_STOP - MIN_START + 1;
    ...
    private NumberPickerView mMinPickerView;
    ...
    private void initInternal(Context context) {
        ...
        mYearPickerView = (NumberPickerView) contentView.findViewById(R.id.pickerYear);
        mMonthPickerView = (NumberPickerView) contentView.findViewById(R.id.pickerMonth);
        mDayPickerView = (NumberPickerView) contentView.findViewById(R.id.pickerDay);
        mHourPickerView = (NumberPickerView) contentView.findViewById(R.id.pickerHour);
        mMinPickerView = (NumberPickerView) contentView.findViewById(R.id.pickerMin);
        ...
        mMinPickerView.setOnValueChangedListener(this);
    }
    ...
    private void setTextHintShow() {
        mYearPickerView.setHintText("年");
        mMonthPickerView.setHintText("月");
        mDayPickerView.setHintText("日");
        mHourPickerView.setHintText("时");
        mMinPickerView.setHintText("分");
    }

    private void setTextHintNull() {
        mYearPickerView.setHintText("");
        mMonthPickerView.setHintText("");
    ...
    private Calendar adjustCalendarByLimit(Calendar c, int yearStart, int yearStop, boolean isGregorian) {
        ...
        if (isGregorian) {
            ...
            if (yearGrego > yearStop) {
                ...
                int daySway = CalendarLunarUtil.getSumOfDayInMonthForGregorianByMonth(yearStop, MONTH_STOP_GREGORIAN);
                c.set(Calendar.DAY_OF_MONTH, daySway);
            }
        } else {
            if (Math.abs(yearGrego - yearStart) < Math.abs(yearGrego - yearStop)) {
                c = new ChineseCalendar(true, yearStart, MONTH_START_LUNAR, DAY_START_LUNAR, HOUR_START, MIN_START);
            } else {
                int daySway = CalendarLunarUtil.getSumOfDayInMonthForLunarByMonthLunar(yearStop, MONTH_STOP_LUNAR_NORMAL);
                c = new ChineseCalendar(true, yearStop, MONTH_STOP_LUNAR_NORMAL, daySway, HOUR_STOP, MIN_STOP);
            }
        }
        return c;
    }
    ...
    public void setThemeColor(int themeColor) {
        ...
        mMinPickerView.setSelectedTextColor(themeColor);
        mMinPickerView.setHintTextColor(themeColor);
        mMinPickerView.setDividerColor(themeColor);
    }
    ...
    public void setNormalColor(int normalColor) {
        ...
        mMinPickerView.setNormalTextColor(normalColor);
    }
    ...
    private void setDisplayValuesForAll(ChineseCalendar cc, boolean isGregorian, boolean anim) {
        ...
        initValuesForMin(cc, isGregorian, anim);
    }

    private void initValuesForMin(ChineseCalendar cc, boolean isGregorian, boolean anim) {
        if (isGregorian) {
            int minSway = cc.get(Calendar.MINUTE);
            setValuesForPickerView(mMinPickerView, minSway, MIN_START, MIN_STOP, mDisplayMin, false, anim);
        } else {
            int minSway = cc.get(Calendar.MINUTE);
            setValuesForPickerView(mMinPickerView, minSway, MIN_START, MIN_STOP, mDisplayMin, false, anim);
        }
    ...
    private void initValuesForH(ChineseCalendar cc, boolean isGregorian, boolean anim) {
        if (isGregorian) {
            int hourSway = cc.get(Calendar.HOUR_OF_DAY);
            setValuesForPickerView(mHourPickerView, hourSway, HOUR_START, HOUR_STOP, mDisplayHour, false, anim);
        } else {
            int hourSway = cc.get(Calendar.HOUR_OF_DAY);
            setValuesForPickerView(mHourPickerView, hourSway, HOUR_START, HOUR_STOP, mDisplayHour, false, anim);
        }
    ...
    private void setDisplayData(boolean isGregorian) {
        ...
        if (isGregorian) {
            ...
            if (mDisplayHour == null) {
                mDisplayHour = new String[HOUR_SPAN];
                for (int i = 0; i < HOUR_SPAN; i++) {
                    mDisplayHour[i] = String.valueOf(HOUR_START + i);
                }
            }
            if (mDisplayMin == null) {
                mDisplayMin = new String[MIN_SPAN];
                for (int i = 0; i < MIN_SPAN; i++) {
                    mDisplayMin[i] = String.valueOf(MIN_START + i);
                }
            }
        } else {
            if (mDisplayYearsLunar == null) {
                ...
                for (int i = 0; i < YEAR_SPAN; i++) {
                    mDisplayYearsLunar[i] = CalendarLunarUtil.getLunarNameOfYear(i + YEAR_START);
                }
            }
            if (mDisplayMonthsLunar == null) {
                mDisplayMonthsLunar = new String[MONTH_SPAN_GREGORIAN];
                for (int i = 0; i < MONTH_SPAN_GREGORIAN; i++) {
                    mDisplayMonthsLunar[i] = CalendarLunarUtil.getLunarNameOfMonth(i + 1);
                }
            }
            if (mDisplayDaysLunar == null) {
                mDisplayDaysLunar = new String[DAY_SPAN_LUNAR];
                for (int i = 0; i < DAY_SPAN_LUNAR; i++) {
                    mDisplayDaysLunar[i] = CalendarLunarUtil.getLunarNameOfDay(i + 1);
                }
            }
            if (mDisplayHour == null) {
                mDisplayHour = new String[HOUR_SPAN];
    ...
    private void initValuesForM(ChineseCalendar cc, boolean isGregorian, boolean anim) {
        ...
        if (isGregorian) {
            ...
            int monthLeap = CalendarLunarUtil.getMonthLeapByYear(cc.get(ChineseCalendar.CHINESE_YEAR));
            if (monthLeap == 0) {
                ...
                monthSway = CalendarLunarUtil.convertMonthLunarToMonthSway(cc.get(ChineseCalendar.CHINESE_MONTH), monthLeap);
                newDisplayedVales = CalendarLunarUtil.getLunarMonthsNamesWithLeap(monthLeap);
            }
        }
    ...
    private void initValuesForD(ChineseCalendar cc, boolean isGregorian, boolean anim) {
        if (isGregorian) {
            ...
            int dayStop = CalendarLunarUtil.getSumOfDayInMonthForGregorianByMonth(cc.get(Calendar.YEAR), cc.get(Calendar.MONTH) + 1);
            ...
            int dayStop = CalendarLunarUtil.getSumOfDayInMonthForLunarByMonthLunar(cc.get(ChineseCalendar.CHINESE_YEAR), cc.get(ChineseCalendar.CHINESE_MONTH));
    ...
    private void passiveUpdateMonthAndDay(int oldYearFix, int newYearFix, boolean isGregorian) {
        ...
        int oldHourSway = mHourPickerView.getValue();
        int oldMinSway = mMinPickerView.getValue();

        if (isGregorian) {
            int newMonthSway = oldMonthSway;
            int oldDayStop = CalendarLunarUtil.getSumOfDayInMonth(oldYearFix, oldMonthSway, true);
            int newDayStop = CalendarLunarUtil.getSumOfDayInMonth(newYearFix, newMonthSway, true);

            if (oldDayStop == newDayStop) {
                if (mOnDateChangedListener != null) {
                    mOnDateChangedListener.onDateChanged(getCalendarData(newYearFix, newMonthSway, oldDaySway, oldHourSway, oldMinSway, isGregorian));
                }
                return;
            }
            ...
            if (mOnDateChangedListener != null) {
                mOnDateChangedListener.onDateChanged(getCalendarData(newYearFix, newMonthSway, newDaySway, oldHourSway, oldMinSway, isGregorian));
            }
            return;
        } else {
            int newMonthSway = 0;

            int newYearFixMonthLeap = CalendarLunarUtil.getMonthLeapByYear(newYearFix);//1.计算当前year是否有闰月
            int oldYearFixMonthLeap = CalendarLunarUtil.getMonthLeapByYear(oldYearFix);//2.计算之前year是否有闰月

            if (newYearFixMonthLeap == oldYearFixMonthLeap) {
                //only update day picker
                newMonthSway = oldMonthSway;

                int oldMonthLunar = CalendarLunarUtil.convertMonthSwayToMonthLunar(oldMonthSway, oldYearFixMonthLeap);
                int newMonthLunar = CalendarLunarUtil.convertMonthSwayToMonthLunar(newMonthSway, newYearFixMonthLeap);
                int oldDayStop = CalendarLunarUtil.getSumOfDayInMonthForLunarByMonthLunar(oldYearFix, oldMonthLunar);
                int newDayStop = CalendarLunarUtil.getSumOfDayInMonthForLunarByMonthLunar(newYearFix, newMonthLunar);
                ...
                //月视图需要更新
                mCurrDisplayMonthsLunar = CalendarLunarUtil.getLunarMonthsNamesWithLeap(newYearFixMonthLeap);

                //优化方案
                int oldMonthLunar = CalendarLunarUtil.convertMonthSwayToMonthLunar(oldMonthSway, oldYearFixMonthLeap);
                int oldMonthLunarAbs = Math.abs(oldMonthLunar);
                newMonthSway = CalendarLunarUtil.convertMonthLunarToMonthSway(oldMonthLunarAbs, newYearFixMonthLeap);
                ...
                //日视图需要更新
                int oldDayStop = CalendarLunarUtil.getSumOfDayInMonth(oldYearFix, oldMonthSway, false);
                int newDayStop = CalendarLunarUtil.getSumOfDayInMonth(newYearFix, newMonthSway, false);
                if (oldDayStop == newDayStop) {
                    if (mOnDateChangedListener != null) {
                        mOnDateChangedListener.onDateChanged(getCalendarData(newYearFix, newMonthSway, oldDaySway, oldHourSway, oldMinSway, isGregorian));
                    }
    ...
    private void passiveUpdateDay(int oldYear, int newYear, int oldMonth, int newMonth, boolean isGregorian) {
        ...
        int oldDayStop = CalendarLunarUtil.getSumOfDayInMonth(oldYear, oldMonth, isGregorian);
        int newDayStop = CalendarLunarUtil.getSumOfDayInMonth(newYear, newMonth, isGregorian);

        if (oldDayStop == newDayStop) {
            if (mOnDateChangedListener != null) {
                mOnDateChangedListener.onDateChanged(getCalendarData(newYear, newMonth, oldDaySway, oldHourSway, oldMinSway, isGregorian));
            }
            return;//不需要更新
        } else {
            int newDaySway = oldDaySway <= newDayStop ? oldDaySway : newDayStop;
            ...
            if (mOnDateChangedListener != null) {
                mOnDateChangedListener.onDateChanged(getCalendarData(newYear, newMonth, newDaySway, oldHourSway, oldMinSway, isGregorian));
            }
            return;
        }
    }
...
//        int year = isGregorian ? cc.get(Calendar.YEAR) : ;
        int year = cc.get(Calendar.YEAR);
        if (cc instanceof ChineseCalendar && !isGregorian) {
            year = ((ChineseCalendar) cc).get(ChineseCalendar.CHINESE_YEAR);
        }
    ...
    private CalendarData getCalendarData(int pickedYear, int pickedMonthSway, int pickedDay, int pickedHour, int pickedMin, boolean mIsGregorian) {
        return new CalendarData(pickedYear, pickedMonthSway, pickedDay, pickedHour, pickedMin, mIsGregorian);
    }
    ...
    public CalendarData getCalendarData() {
        ...
        int pickedMin = mMinPickerView.getValue();
        return new CalendarData(pickedYear, pickedMonthSway, pickedDay, pickedHour, pickedMin, mIsGregorian);
    }
    ...
    public static class CalendarData {
         ...
         */
        public CalendarData(int pickedYear, int pickedMonthSway, int pickedDay, int pickedHour, int pickedMin, boolean isGregorian) {
            this.pickedYear = pickedYear;
            this.pickedMonthSway = pickedMonthSway;
            this.pickedDay = pickedDay;
            this.pickedHour = pickedHour;
            this.pickedMin = pickedMin;
        ...
        private void initChineseCalendar() {
            if (isGregorian) {
                chineseCalendar = new ChineseCalendar(pickedYear, pickedMonthSway - 1, pickedDay, pickedHour, pickedMin);//公历日期构造方法
            } else {
                int y = pickedYear;
                int m = CalendarLunarUtil.convertMonthSwayToMonthLunarByYear(pickedMonthSway, pickedYear);
                int d = pickedDay;
                int h = pickedHour;
                int min = pickedMin;

                chineseCalendar = new ChineseCalendar(true, y, m, d, h, min);
            }
        }
